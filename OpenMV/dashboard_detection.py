...
while True:
    ...
    for i, detection_list in enumerate(net.predict([img], callback=fomo_post_process)):
        if i == 0:
            continue  # skip background or class 0 if not used

        if len(detection_list) == 0:
            continue

        # Only process first detected object for now (e.g. drowning person)
        ...
        # Draw detection box and circle on image
        ...
        break  # Only handle first detection

    # If no detection, reset dx, dy
    if not detected:
        dx, dy = 0, 0

    # Print dx/dy for Pi to read
    print(f"dx:{dx};dy:{dy}")

    # Send dx, dy over USB serial for Raspberry Pi to consume
    status_line = f"dx:{dx};dy:{dy}\n"
    usb.write(status_line)

    # Send compressed JPEG image for live feed
